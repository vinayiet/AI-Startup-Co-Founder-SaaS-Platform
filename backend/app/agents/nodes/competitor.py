...
async def competitor_node(state: AgentState) -> AgentState:
    ...
    comp_id_system_prompt = (
        ...
        "identify exactly 3 distinct, real-world competitor companies that actually exist in this SPECIFIC niche. "
        "CRITICAL: Do NOT pick generic enterprise giants (like Workday, Salesforce, or ADP) unless the startup is explicitly an enterprise tool. "
        "You must find DIRECT competitors targeting the exact same user base and use-case.\n"
    ...
    system_prompt = (
        ...
        "3. If pricing information for a competitor is not explicitly present in the search snippets, DO NOT output 'data not found'. "
        "Instead, use your internal knowledge base to provide their ACTUAL known pricing tiers (e.g. '$15/month basic'). "
        "Only append '(estimated)' if it is a highly obscure company with no public pricing. "
        "For large public companies (like Workday, ADP, etc.), you MUST know and state their standard enterprise pricing models.\n"
        "4. For each competitor, you MUST provide a valid, clickable 'source_url'. If the exact pricing link isn't in the search snippet, "
        "reconstruct their official pricing page URL (e.g., 'https://www.company.com/pricing') and set 'last_verified' to 'June 2026'.\n"
